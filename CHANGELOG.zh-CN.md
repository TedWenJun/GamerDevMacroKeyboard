...
### 修复
- 英文 Web 界面中默认的层、功能、分类与控件名称（含键帽上的功能名）仍显示为中文；现在仍是默认值的名称都随界面语言显示（`GET /api/names`）。键帽上的英文名称按宽度截断。

### 变更
- 灯光方向按钮显示 ↻ / ↺，不再显示“顺 / 逆”（说明在悬停提示中）。
- `POST /api/config/reset?lang=zh|en` 按指定语言恢复默认配置；Web 界面传入当前界面语言。
